...
async def run_stage1(idea: str) -> Stage1Result:
    ...
    for wave_number, roles in enumerate(plan.waves, start=1):
        ...
        results = await asyncio.gather(
            *[
                _run_worker(project.id, wave_id, role, idea, dict(artifacts), None, semaphore)
                for role in roles
            ]
        )
        wave_failed = False
        ...
        await _complete_wave(wave_id, WaveStatus.FAILED if wave_failed else WaveStatus.DONE)
        if wave_failed:
    ...
    if report.overall_verdict == "needs_rework":
        # Collect issues per affected artifact and rerun those roles once.
        affected_by_role: dict[AgentRole, list[str]] = {}
        name_to_role = {
            role.value: role
            for role in AgentRole
            if role not in (AgentRole.LEAD, AgentRole.REVIEWER)
        }
        ...
        # One cycle only — we accept whatever comes back.

...
2. Test Agent writes tests (Claude Code CLI, own branch)
3. Lead creates code task (depends on test task)
4. Code Agent implements (Claude Code CLI, own branch)
5. Tests run automatically → green = done, red = iterate
6. Lead merges branch when green
...
- **CLAUDE.md** — synthesized from all docs: project overview, tech stack, conventions, architecture rules
...
User submits idea + GitHub token
  ...
  → Initial commit: project idea + metadata
  → Stage 1: each doc committed as generated
  → Stage 2: agents work on branches, Lead merges
```
...
User submits idea + repo URL + GitHub token
  → Clone the repo
  → Stage 1: docs committed to a /docs branch
  → Stage 2: agents work on branches from main
```
